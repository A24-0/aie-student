...
def compute_quality_flags(
    ...
    zero_values_threshold: float = 0.5,
) -> Dict[str, Any]:
    """
    ...
    # Проверка на дубликаты ID (колонки с названиями, содержащими id)
    suspicious_id_duplicates: List[str] = []
    for col in summary.columns:
        col_name_lower = col.name.lower()
        if "id" in col_name_lower and col.unique < summary.n_rows:
            # Если это похоже на ID, но уникальных значений меньше, чем строк
    ...
    for col in summary.columns:
        if col.is_numeric and col.non_null > 0:
            # Подсчитываем долю нулей
            col_series = df[col.name]
            zero_count = (col_series == 0).sum()
            zero_share = float(zero_count / col.non_null) if col.non_null > 0 else 0.0
            if zero_share >= zero_values_threshold:
                many_zero_cols.append(col.name)
    flags["has_many_zero_values"] = len(many_zero_cols) > 0
    flags["many_zero_columns"] = many_zero_cols

    # Обновлённый «скор» качества с учётом новых факторов
    score = 1.0
