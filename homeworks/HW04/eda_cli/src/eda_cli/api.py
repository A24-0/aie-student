...
# ---------- Собственный эндпоинт: /quality-flags-from-csv ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, bool]]:
    """
    Возвращает полный набор флагов качества из загруженного CSV-файла.
    Включает все эвристики качества данных, реализованные в HW03:
    ...
    # Используем EDA-ядро для получения всех флагов
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    quality_flags = compute_quality_flags(summary, missing_df, df)

    # Формируем ответ только с булевыми флагами (как в примере из задания)
    flags_response: dict[str, bool] = {
    ...
    print(
        ...
        f"flags_count={len(flags_response)}"
    )

    return {"flags": flags_response}