...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"**Настройки анализа:**\n")
        f.write(f"- Порог доли пропусков для проблемных колонок: **{min_missing_share:.1%}**\n")
        f.write(f"- Количество top-значений для категорий: **{top_k_categories}**\n")
        f.write(f"- Максимум гистограмм: **{max_hist_columns}**\n\n")
        
        f.write(f"**Результаты проверки качества:**\n")
        ...
        if quality_flags['has_suspicious_id_duplicates']:
            f.write(f"  - Колонки с дубликатами ID: {', '.join(quality_flags['suspicious_id_columns'])}\n")
        ...
        if quality_flags['has_many_zero_values']:
            f.write(f"  - Колонки с большим количеством нулей: {', '.join(quality_flags['many_zero_columns'])}\n")
        f.write("\n")
        ...
        if missing_df.empty:
            ...
            # Добавляем список проблемных колонок по пропускам
            ...
            if not problematic_cols.empty:
                ...
                for col_name in problematic_cols.index:
                    missing_share_val = problematic_cols.loc[col_name, "missing_share"]
                    f.write(f"- `{col_name}`: {missing_share_val:.1%}\n")
                f.write("\n")
    ...
    if json_summary:
        problematic_cols_list: list[str] = []
        if not missing_df.empty:
            problematic_missing = missing_df[missing_df["missing_share"] >= min_missing_share]
            problematic_cols_list.extend(problematic_missing.index.tolist())
        
        # Добавляем колонки из эвристик качества
        if quality_flags.get("has_constant_columns"):
            problematic_cols_list.extend(quality_flags.get("constant_columns", []))
        if quality_flags.get("has_suspicious_id_duplicates"):
            problematic_cols_list.extend(quality_flags.get("suspicious_id_columns", []))
        if quality_flags.get("has_many_zero_values"):
            problematic_cols_list.extend(quality_flags.get("many_zero_columns", []))
        
        # Убираем дубликаты
        problematic_cols_list = list(set(problematic_cols_list))
        
        json_summary_data = {
            "n_rows": summary.n_rows,
            "n_cols": summary.n_cols,
            "quality_score": quality_flags["quality_score"],
            "problematic_columns": problematic_cols_list,
            "quality_flags": {
